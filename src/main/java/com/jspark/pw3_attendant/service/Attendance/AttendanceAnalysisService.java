...
public class AttendanceAnalysisService {
    ...
    public List<AbsenteeResponse> findLongTermAbsentees(LocalDate startDate, LocalDate endDate) {
        ...
        List<Attendance> attendances = attendanceRepository.findSundayAttendanceByDateBetweenAndStatusIn(startDate, endDate,
    ...
    public List<AbsenteeResponse> findConsecutiveAbsenceStudents(int weeks) {
        ...
        for (int i = 0; i < weeks; i++) {
            LocalDate weekEnd = today.minusWeeks(i).with(DayOfWeek.SATURDAY);
            LocalDate weekStart = weekEnd.with(DayOfWeek.SUNDAY);

            List<Attendance> weeklyAttendances = attendanceRepository.findSundayAttendanceByDateBetweenAndStatusIn(weekStart, weekEnd,
                    List.of(Attendance.AttendanceStatus.ATTEND, Attendance.AttendanceStatus.LATE));
